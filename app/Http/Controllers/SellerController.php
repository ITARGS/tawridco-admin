...
class SellerController extends BaseController
{
    public function index()
    {
        ...
        if ($categoryIds != "") {
            ...
            $category_product_count = Category::select('id', $categoryNameField, DB::raw('(SELECT count(id) from `products` WHERE products.category_id = categories.id and categories.id IN(' . $categoryIds . ') and products.seller_id = ' . $seller_id . ') AS product_count'))
            ...
            $category_product_count = Category::select('id', $categoryNameField)->get()->makeHidden(['image_url']);
        }
    ...
    public function getProducts(Request $request)
    {
        ...
        $products = \DB::select("SELECT p.id AS product_id, p.*, p.name_en, p.name_ar, p.seller_id, p.status, p.tax_id, p.image,
    ...
    public function getWeeklySales()
    {
        ...
        $curdate = date('Y-m-d');
        $orders = Order::select(
            DB::raw('ROUND(SUM(final_total), 2) AS total_sale'),
    ...
    public function getCategories(Request $request)
    {
        ...
        if (isset($request->category_id)) {
            $categories =  Category::where('parent_id', $category_id)->orderBy($categoryNameField, 'ASC')->get();
        } else {
            $categories =  Category::whereIn('id', explode(",", $seller_categories))->where('parent_id', $category_id)->orderBy($categoryNameField, 'ASC')->get();
        }
        return CommonHelper::responseWithData($categories);
    }
    ...
    public function getMainCategories(Request $request)
    {
        ...
        $query = Category::orderBy($categoryNameField, 'ASC');
