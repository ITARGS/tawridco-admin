<template>
  <header class="mb-3" :style="mainStyle">
    <nav class="navbar navbar-expand-lg">
      <div class="container-fluid">
        <a @click="openBAR" href="javascript:void(0)" class="burger-btn d-block">
          <i class="fa fa-bars" aria-hidden="true"></i>
        </a>

        <div class="d-flex justify-content-start align-items-center">
          <div class="me-2" v-if="$isDemo == 1 && windowWidth < this.$mobileWidth">
            <span class="badge bg-danger">{{ __("demo_mode") }}</span>
          </div>
          <a
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
              ...
              <div class="d-flex gap-2 align-items-center mt-2">
                ...
                >
                  ...
                  <i
                    v-else
                    class="fa fa-refresh"
                    :class="{ 'fa-spin': isSystemRefreshing }"
                  ></i>
                </button>
                ...
                >
                  ...
                  <template #default>
                    <h6>{{ __("welcome_message") }}</h6>

                    <span>{{ __("cache_clear") }}</span
                    >, <span>{{ __("config_clear") }}</span
                    >, <span>{{ __("route_clear") }}</span
                    >, <span>{{ __("view_clear") }}</span
                    >,
            ...
            <li class="nav-item dropdown me-1">
              ...
              >
                ...
                <li>
                  <a class="dropdown-item" href="#">{{ __("no_new_message") }}</a>
                </li>
              </ul>
            </li>
            <li class="nav-item dropdown me-1">
                    ...
                    <div class="notification-text ms-4">
                      <p class="notification-title font-bold">
                        <router-link :to="'/orders/view/' + notification.data.order_id">
                          {{ notification.data.text }}</router-link
                        >
                      </p>
          ...
          <div class="dropdown">
            ...
            <ul
              class="dropdown-menu dropdown-menu-lg-end mr-5"
...
export default {
  ...
  methods: {
    ...
    getLanguage() {
      ...
      axios
        ...
        .then((response) => {
          ...
          const currentRTL = window.localStorage.getItem("isRTL");
          this.selectedLanguage = currentLang;
          this.isRTL = currentRTL === "true";
          if (currentLang || currentLanguage || currentRTL) {
          } else {
    ...
    getNotifications(event) {
      axios.get(this.$apiUrl + "/get_top_notifications").then((response) => {
    ...
    markAsReadNotification(notification) {
      if (notification.read_at == null) {
        axios
          .get(this.$apiUrl + "/notification_read?id=" + notification.id)
          .then((response) => {
            this.getNotifications();
          });
      }
