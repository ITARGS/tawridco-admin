              ...
              <b-row class="table-responsive">
                ...
                >
                  ...
                  <template #cell(other_info)="row">
                    ...
                    <div class="mainPopover">
                      ...
                      >
                        ...
                        <table class="table table-borderless">
                          <tr>
                            <th>{{ __("commercial_registration") }}</th>
                            <td>: {{ row.item.commercial_registration_number }}</td>
                          </tr>
                          <tr>
                            <th>{{ __("commercial_registration_expire_date") }}</th>
                            <td id="subPopover">
                              : {{ row.item.commercial_registration_expire_date }}
                            </td>
                          </tr>
                          <tr>
                            <th>{{ __("municipality_license") }}</th>
                            <td>: {{ row.item.license_number }}</td>
                          </tr>
                          <tr>
                            <th>{{ __("municipality_expire_number") }}</th>
                            <td>: {{ row.item.license_number_expire_date }}</td>
                          </tr>
                          <tr>
                            <th>{{ __("tax_no") }}</th>
                        ...
                        <p>
                          <a
                            target="_blank"
                            :href="row.item.municipality_license_url"
                            class="badge bg-success"
                          >
                            <i class="fa fa-eye"></i>
                            {{ __("municipality_license_image") }}</a
                          >
                        </p>
                        <p>
                          <a
                            target="_blank"
                            :href="row.item.commercial_registration_url"
                            class="badge bg-success"
                          >
                            <i class="fa fa-eye"></i>
                  ...
                  <template #cell(status)="row">
                    <label v-if="row.item.status == 0" class="badge bg-primary">{{
                      __("registered")
                    }}</label>
                  ...
                  <template #cell(require_products_approval)="row">
                    <label
                      v-if="row.item.require_products_approval == 1"
                      class="badge bg-success"
                      >{{ __("yes") }}</label
                    >
                    <label
                      v-else-if="row.item.require_products_approval == 0"
        ...
        .then(async (result) => {
          if (result.value) {
            ...
            if (selectedStatus === 1 || (selectedStatus === 2 && remarks !== "")) {
              ...
              axios
                .post(this.$apiUrl + "/sellers/update_status", postData)
                .then((response) => {
                  this.isLoading = false;
                  let data = response.data;
                  this.getRecords();
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.value) {
            ...
            axios.post(this.$apiUrl + "/sellers/delete", postData).then((response) => {
              this.isLoading = false;
              let data = response.data;
    ...
    updateSellerCommission() {
      this.$swal
        ...
        .then((result) => {
          if (result.value) {
            this.isLoading = true;
            axios.get(this.$apiUrl + "/sellers/updateCommission").then((response) => {
              let data = response.data;
              if (data.status === 1) {
                this.showSuccess(data.message);
              } else {
